...
bool CanopenInterface::sdoDownload(RuntimeCommand command, uint8_t subindex, uint32_t data) {
  ...
  frame.data[0] =
      (sdo_command_ << 4) | (CanopenInterface::RUNTIME_COMMAND_MAP_.at(command).number_of_unused_bytes << 2);
...
uint32_t CanopenInterface::sdoUpload(RuntimeQuery query, uint8_t subindex) {
  ...
  query_frame.data[0] =
      (sdo_query_ << 4) | (CanopenInterface::RUNTINE_QUERY_MAP_.at(query).number_of_unused_bytes << 2);
